...
interface PhotosUploaderProps {
  addedPhotos: string[];
  setAddedPhotos: (arr: string[]) => void;
}
const PhotosUploader: React.FC<PhotosUploaderProps> = ({
  ...
  const addPhotoByLink = async (
    ...
    ev.preventDefault();
    const { data } = await axios.post("/place/uploadByLink", {
      link: photoLink,
    });
    setPhotoLink("");
    const array = [...addedPhotos];
    setAddedPhotos([data, ...array]);
  };
  ...
  const uploadPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      const { data: filenames } = await axios.post("/place/upload", data, {
        headers: { "Content-type": "multipart/form-data" },
      });
      const array = [...addedPhotos];
      setAddedPhotos([...filenames, ...array]);
    }
  };
  const removePhoto = (
    ev: React.MouseEvent<HTMLButtonElement, MouseEvent>,
    ...
    ev.preventDefault();
    const remainingPhotos = addedPhotos.filter((photo) => photo !== fileName);
    setAddedPhotos(remainingPhotos);
  };
  const selectAsMainPhoto = (
    ...
    ev.preventDefault();
    const remainingPhotos = addedPhotos.filter((photo) => photo !== fileName);
    setAddedPhotos([fileName, ...remainingPhotos]);
  };
  return (
    <>
        ...
        {addedPhotos.length > 0 &&
          addedPhotos.map((fileName) => (
            <div className="h-32 flex relative" key={fileName}>
              ...
              />
              <button
                onClick={(ev) => removePhoto(ev, fileName)}
              ...
              <button
                onClick={(ev) => selectAsMainPhoto(ev, fileName)}
